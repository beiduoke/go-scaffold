...
func toMenuModel(d *biz.Menu) *SysMenu {
	if d == nil {
		return nil
	}
	sysData := &SysMenu{
...
func toMenuBiz(d *SysMenu) *biz.Menu {
	if d == nil {
		return nil
	}
	data := &biz.Menu{
...
func (r *MenuRepo) ListByIDs(ctx context.Context, id ...uint) (menus []*biz.Menu, err error) {
	...
	for _, v := range sysMenus {
		menus = append(menus, toMenuBiz(v))
	}
	return
}
...
func (r *MenuRepo) ListByName(ctx context.Context, name string) ([]*biz.Menu, error) {
	...
	for _, v := range sysMenus {
		bizMenus = append(bizMenus, toMenuBiz(v))
	}
	return bizMenus, nil
}

func (r *MenuRepo) Delete(ctx context.Context, g *biz.Menu) error {
	result := r.data.DB(ctx).Delete(toMenuModel(g))
