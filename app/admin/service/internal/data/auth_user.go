...
func NewSecurityUser(logger log.Logger, data *Data, authRepo biz.AuthRepo) authn.SecurityUserCreator {
	...
	return func(ac *authn.AuthClaims) authn.SecurityUser {
		...
		return &securityUser{options: SecurityOptions{data: data, log: log, authRepo: authRepo.(*AuthRepo), authClaims: ac}}
	}
}
...
func (su *securityUser) ParseFromContext(ctx context.Context) error {
	...
	authUser, err := su.options.authRepo.GetLoginUUIDCache(ctx, su.options.authClaims.Subject)
	if err != nil {
