...
func RegisterWebHTTPServer(s *http.Server, srv WebHTTPServer) {
	r := s.Route("/")
	r.POST("/web/v1/login", _Web_Login1_HTTP_Handler(srv))
	r.POST("/web/v1/logout", _Web_Logout1_HTTP_Handler(srv))
	r.GET("/web/v1/users", _Web_ListUser1_HTTP_Handler(srv))
}

func _Web_Login1_HTTP_Handler(srv WebHTTPServer) func(ctx http.Context) error {
...
func _Web_Logout1_HTTP_Handler(srv WebHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in LogoutReq
		if err := ctx.Bind(&in); err != nil {
			return err
