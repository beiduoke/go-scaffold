...
const OperationApiHandleRoleDept = "/api.server.v1.Api/HandleRoleDept"
...
func RegisterApiHTTPServer(s *http.Server, srv ApiHTTPServer) {
	...
	r.POST("/v1/roles/{id}/depts", _Api_HandleRoleDept0_HTTP_Handler(srv))
...
func _Api_HandleRoleDept0_HTTP_Handler(srv ApiHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in HandleRoleDeptReq
		if err := ctx.Bind(&in.Data); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationApiHandleRoleDept)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.HandleRoleDept(ctx, req.(*HandleRoleDeptReq))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*HandleRoleDeptReply)
		return ctx.Result(200, reply)
	}
}
...
type ApiHTTPClient interface {
	...
	HandleRoleDept(ctx context.Context, req *HandleRoleDeptReq, opts ...http.CallOption) (rsp *HandleRoleDeptReply, err error)
...
func (c *ApiHTTPClientImpl) HandleRoleDept(ctx context.Context, in *HandleRoleDeptReq, opts ...http.CallOption) (*HandleRoleDeptReply, error) {
	var out HandleRoleDeptReply
	pattern := "/v1/roles/{id}/depts"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationApiHandleRoleDept))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in.Data, &out, opts...)
	if err != nil {
		return nil, err
