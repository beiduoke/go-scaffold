...
const OperationUserServiceExistUserByName = "/admin.service.v1.UserService/ExistUserByName"
...
func RegisterUserServiceHTTPServer(s *http.Server, srv UserServiceHTTPServer) {
	...
	r.POST("/v1/users/existByName", _UserService_ExistUserByName0_HTTP_Handler(srv))
}
...
func _UserService_ExistUserByName0_HTTP_Handler(srv UserServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in ExistUserByNameRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationUserServiceExistUserByName)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.ExistUserByName(ctx, req.(*ExistUserByNameRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*ExistUserByNameResponse)
		return ctx.Result(200, reply)
	}
}
...
type UserServiceHTTPClient interface {
	...
	ExistUserByName(ctx context.Context, req *ExistUserByNameRequest, opts ...http.CallOption) (rsp *ExistUserByNameResponse, err error)
...
func (c *UserServiceHTTPClientImpl) ExistUserByName(ctx context.Context, in *ExistUserByNameRequest, opts ...http.CallOption) (*ExistUserByNameResponse, error) {
	var out ExistUserByNameResponse
	pattern := "/v1/users/existByName"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationUserServiceExistUserByName))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
