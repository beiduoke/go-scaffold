...
func wireApp(confServer *conf.Server, auth *conf.Auth, confData *conf.Data, logger log.Logger) (*kratos.App, func(), error) {
	...
	bizBiz := biz.NewBiz(logger, transaction, iEnforcer, domainRepo, userRepo)
	authUsecase := biz.NewAuthUsecase(bizBiz, auth)
	userUsecase := biz.NewUserUsecase(bizBiz, auth)
	domainUsecase := biz.NewDomainUsecase(bizBiz)
	authorityRepo := data.NewAuthorityRepo(dataData, iEnforcer, logger)
	authorityUsecase := biz.NewAuthorityUsecase(bizBiz, authorityRepo)
	menuRepo := data.NewMenuRepo(dataData, logger)
	menuUsecase := biz.NewMenuUsecase(bizBiz, menuRepo)
	apiRepo := data.NewApiRepo(dataData, logger)
	apiUsecase := biz.NewApiUsecase(bizBiz, apiRepo)
