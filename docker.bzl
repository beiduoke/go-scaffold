...
def publish_service(service_name, repository_name = "", repository_version = "", publish = False):
    ...
    app_path = "/bin".format(service_name)
    conf_path = "/configs".format(service_name)
    ...
    native.alias(
        ...
        actual = "//cmd/server:server".format(service_name),
    ...
    native.filegroup(
        ...
        srcs = native.glob(["configs/**".format(service_name)]),
